...
_UPDATE() {
	...
	# 这里可以自己添加、修改 CloudflareST 的运行参数 https://worktest.frankbu.top/test
	/usr/bin/cdnspeedtest -o "result_hosts.txt" -url https://cloudflare.cdn.openbsd.org/pub/OpenBSD/7.3/src.tar.gz
	...
	[[ $(cat result_hosts.txt|wc -l) > 5 ]] && echo "CloudflareST 测速结果没有找到一个完全满足条件的 IP，重新测速..." && _UPDATE
