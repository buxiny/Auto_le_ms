...
_UPDATE() {
	echo -e "开始测速..."
	NOWIP=$(head -1 nowip_hosts-v6.txt)

	# 这里可以自己添加、修改 CloudflareST 的运行参数
	/usr/bin/cdnspeedtest -f "ipv6.txt" -o "result_hosts-v6.txt" -url https://cloudflare.cdn.openbsd.org/pub/OpenBSD/7.3/src.tar.gz

	# 如果需要 "找不到满足条件的 IP 就一直循环测速下去"，那么可以将下面的两个 exit 0 改为 _UPDATE 即可
	[[ ! -e "result_hosts-v6.txt" ]] && echo "CloudflareST 测速结果 IP 数量为 0，跳过下面步骤..." && exit 0
	...
	#[[ $(cat result_hosts-v6.txt|wc -l) > 11 ]] && echo "CloudflareST 测速结果没有找到一个完全满足条件的 IP，重新测速..." && _UPDATE


	BESTIP=$(sed -n "2,1p" result_hosts-v6.txt | awk -F, '{print $1}')
	if [[ -z "${BESTIP}" ]]; then
		echo "CloudflareST 测速结果 IP 数量为 0，跳过下面步骤..."
		exit 0
	fi
	echo ${BESTIP} > nowip_hosts-v6.txt
	echo -e "\n旧 IP 为 ${NOWIP}\n新 IP 为 ${BESTIP}\n"
...
#	sed -i 's/'${NOWIP}'/'${BESTIP}'/g' /mnt/docker/mosdns/config.yaml
